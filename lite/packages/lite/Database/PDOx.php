...
use PDO;
use PDOException;
use Lite\Contracts\Database\PDOxInterface as PDOxInterface;
use Lite\Storage\Cache;
...
class PDOx implements PDOxInterface
{
    ...
    public function __construct($config = null)
    {
        ...
        $config['collation'] = $config['collation'] ?? 'utf8_general_ci';
        $config['port'] = $config['port'] ?? (strstr($config['host'], ':') ? explode(':', $config['host'])[1] : '');
        $this->prefix = $config['prefix'] ?? '';
        $this->cacheDir = $config['cachedir'] ?? app_path('storage/cache/');
        ...
        try {
        	...
        	if(!$this->debug){
		        view('404',['error'=>"Could not connect to Database! 💻"]);
		        die();
	        }
        }
        return $this->pdo;
	 ...
	 * @return $this
	 */
    public function whereIn($field, array $keys, $type = ''){

    	$this->where = '';
    	$this->in($field,$keys,$type,'');
     ...
     *
     * @return mixed|string|Eloquent|Collection
     */
     ...
     * @param string $argument
     *
     * @return $this|mixed|Collection|Eloquent
     */
    public function query($query, $all = true, $type = null, $argument = null)
    {
        ...
        if($this->result){
        	if($all)
        		return $this->toCollection($this->result);
        	else return new $this->eloquent($this->result);
        }
        return $this->result;
    }
    ...
    private function toCollection($collection){
    	//Change raw data to eloquent models
    	foreach ($collection as &$value)
    		$value = new $this->eloquent($value);
        $collect = new Collection((array)$collection);
        if(!$this->dicBy)
        	return $collect;
        return $collect->groupBy($this->dicBy);
    }
...
//    		print_r($data);
        return ($data === null && !($data instanceof \ArrayAccess) ) ? 'NULL' : $this->pdo->quote($data);
    }
